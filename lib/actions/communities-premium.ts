'use server'

import { createClient, createServiceClient } from '@/lib/supabase/server'
import { revalidatePath } from 'next/cache'

// 1. Dynamic Accent Color Updates
export async function updateCommunityAccent(communityId: string, color: string) {
  const supabase = await createClient()
  const serviceClient = await createServiceClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { error: 'Giriş yapmalısınız.' }

  // Verify that the user is owner or moderator
  const { data: membership } = await supabase
    .from('community_members')
    .select('role, status')
    .eq('community_id', communityId)
    .eq('user_id', user.id)
    .single()

  if (!membership || membership.status !== 'approved' || (membership.role !== 'owner' && membership.role !== 'moderator')) {
    return { error: 'Bu toplulukta renk ayarını değiştirme yetkiniz yok.' }
  }

  const { error } = await serviceClient
    .from('communities')
    .update({ accent_color: color })
    .eq('id', communityId)

  if (error) return { error: error.message }

  const { data: community } = await supabase.from('communities').select('slug').eq('id', communityId).single()
  if (community) {
    revalidatePath(`/communities/${community.slug}`)
  }
  revalidatePath('/communities')
  revalidatePath('/feed')

  return { success: true }
}

// 2. Gamification XP Rewards
export async function rewardXP(userId: string, amount: number) {
  try {
    const serviceClient = await createServiceClient()
    
    // Get current XP
    const { data: profile, error: getError } = await serviceClient
      .from('profiles')
      .select('xp')
      .eq('id', userId)
      .single()

    if (getError || !profile) return { error: 'Profil bulunamadı.' }

    const newXp = (profile.xp || 0) + amount

    const { error: updateError } = await serviceClient
      .from('profiles')
      .update({ xp: newXp })
      .eq('id', userId)

    if (updateError) return { error: updateError.message }
    return { success: true, newXp }
  } catch (err: any) {
    return { error: err.message }
  }
}

// 3. Lounge Sticky Notes Actions
export async function addLoungeNote(communityId: string, content: string, color: string = 'yellow') {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { error: 'Giriş yapmalısınız.' }

  // Verify membership in community
  const { data: membership } = await supabase
    .from('community_members')
    .select('status')
    .eq('community_id', communityId)
    .eq('user_id', user.id)
    .single()

  if (!membership || membership.status !== 'approved') {
    return { error: 'Not eklemek için bu topluluğa üye olmalısınız.' }
  }

  // Generate random positions between 5% and 80% to fit neatly on screen
  const xPos = Math.floor(Math.random() * 75) + 5
  const yPos = Math.floor(Math.random() * 70) + 10

  const { data: note, error } = await supabase
    .from('community_lounge_notes')
    .insert({
      community_id: communityId,
      user_id: user.id,
      content,
      color,
      x_pos: xPos,
      y_pos: yPos
    })
    .select()
    .single()

  if (error) return { error: error.message }

  // Reward contributor with +5 XP
  await rewardXP(user.id, 5)

  return { success: true, note }
}

export async function updateLoungeNotePosition(noteId: string, xPos: number, yPos: number) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { error: 'Giriş yapmalısınız.' }

  // Get note to check creator or admin
  const { data: note } = await supabase
    .from('community_lounge_notes')
    .select('user_id, community_id')
    .eq('id', noteId)
    .single()

  if (!note) return { error: 'Not bulunamadı.' }

  const isCreator = note.user_id === user.id

  let isAdmin = false
  if (!isCreator) {
    const { data: membership } = await supabase
      .from('community_members')
      .select('role')
      .eq('community_id', note.community_id)
      .eq('user_id', user.id)
      .single()
    isAdmin = membership?.role === 'owner' || membership?.role === 'moderator'
  }

  if (!isCreator && !isAdmin) {
    return { error: 'Bu notun konumunu değiştirme yetkiniz yok.' }
  }

  const { error } = await supabase
    .from('community_lounge_notes')
    .update({ x_pos: xPos, y_pos: yPos })
    .eq('id', noteId)

  if (error) return { error: error.message }
  return { success: true }
}

export async function deleteLoungeNote(noteId: string) {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { error: 'Giriş yapmalısınız.' }

  const { data: note } = await supabase
    .from('community_lounge_notes')
    .select('user_id, community_id')
    .eq('id', noteId)
    .single()

  if (!note) return { error: 'Not bulunamadı.' }

  const isCreator = note.user_id === user.id

  let isAdmin = false
  if (!isCreator) {
    const { data: membership } = await supabase
      .from('community_members')
      .select('role')
      .eq('community_id', note.community_id)
      .eq('user_id', user.id)
      .single()
    isAdmin = membership?.role === 'owner' || membership?.role === 'moderator'
  }

  if (!isCreator && !isAdmin) {
    return { error: 'Bu notu silme yetkiniz yok.' }
  }

  const { error } = await supabase
    .from('community_lounge_notes')
    .delete()
    .eq('id', noteId)

  if (error) return { error: error.message }
  return { success: true }
}
