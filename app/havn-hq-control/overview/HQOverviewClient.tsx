...
export default function HQOverviewClient({
  ...
  const activeChartData = useMemo(() => {
    ...
    if (timeRange === '24s') {
      ...
      multiplier = 18
      dataPoints = dataPoints.map((d, i) => ({ ...d, name: `${i+1} Haz` }))
  ...
  const handleExportPDF = () => {
    ...
    let html = `
      <html>
        <head>
          <title>HAVN HQ Sistem Raporu - ${new Date().toISOString().split('T')[0]}</title>
          <style>
        ...
        <body>
          <h1>HAVN HQ - SİSTEM PERFORMANS RAPORU</h1>
          <div class="meta">Oluşturulma Tarihi: ${timestamp} | HAVN HQ Yetkili Raporlama Çekirdeği</div>
          
          <h2>1. Sistem Sağlığı & Telemetri</h2>
          <div class="grid">
            <div class="card">
              <div class="card-title">CPU Tüketimi</div>
            ...
            <div class="card">
              <div class="card-title">Ağ Gecikmesi</div>
          ...
          <h2>2. Topluluk İstatistikleri</h2>
          <div class="grid">
            <div class="card">
            ...
            <div class="card">
              <div class="card-title">Aktif & Çevrimiçi</div>
              <div class="card-value">${stats.onlineUsers}</div>
            </div>
            <div class="card">
              <div class="card-title">Gönderiler / Yorumlar</div>
              <div class="card-value">${stats.totalPosts} Gönderi | ${stats.totalComments} Yorum</div>
            </div>
            <div class="card">
              <div class="card-title">Etkileşimler (Beğeni)</div>
              <div class="card-value">${stats.totalLikes} Beğeni</div>
            </div>
          </div>

          <h2>3. Son Sistem Günlükleri (Audit Log)</h2>
          <table>
            <thead>
              <tr>
                <th style="width: 120px;">Zaman</th>
                <th style="width: 150px;">Yetkili</th>
          ...
          <h2>4. Aktif Kullanıcı Listesi Önizleme</h2>
          <table>
            <thead>
              <tr>
    ...
    setActionMsg("Performans raporu PDF yazdırma penceresine aktarıldı.")
  ...
  const handleExportCSV = () => {
    setShowExportMenu(false)
    let csvContent = "Kullanici Adi,Isim,Soyisim,Rol,Level,XP,Uyari Sayisi,Post Sayisi,Kayit Tarihi,Ulke\n"
    users.forEach((u) => {
    ...
    setActionMsg("Kullanıcı verileri CSV olarak başarıyla indirildi.")
    setTimeout(() => setActionMsg(null), 3000)
  }
  ...
  const actionLabels: Record<string, string> = {
    ...
    note_status: 'Durum Değişti',
    note_delete: 'Not Silindi',
    role_change: 'Rol Değişimi',
    user_warn: 'Uyarı Verildi',
    verification_toggle: 'Tik Değişti',
    xp_award: 'XP Ödülü'
  }
    ...
    <div className="w-full p-6 md:p-8 space-y-6 md:space-y-8 select-none text-slate-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-4 border-b border-white/5">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest mb-1 text-slate-400 flex items-center gap-1.5">
            ...
            <span>&gt;</span>
            <span className="text-violet-400 font-extrabold">Genel Durum</span>
          </p>
          <h1 className="text-2xl md:text-3xl font-black text-white tracking-tight">Genel Durum</h1>
          <p className="text-xs text-slate-400 mt-1">
            Tüm sistem, topluluk ve moderasyon metriklerini tek bakışta gör.
          </p>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-1.5 text-slate-400 select-none">
          ...
          <h3 className="text-xs font-black uppercase tracking-widest text-slate-300">Sistem Sağlığı</h3>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="rounded-2xl p-5 bg-card/45 border border-white/[0.04] relative overflow-hidden flex flex-col justify-between min-h-[125px]">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Network Hızı</span>
              <Activity size={14} className="text-emerald-500" />
            </div>
            <div className="my-2">
              <p className="text-2xl font-black text-white">{stats.latency}ms <span className="text-xs text-slate-500">Kararlı</span></p>
              <div className="w-full bg-white/5 rounded-full h-1 mt-2.5 overflow-hidden border border-white/5">
                <div className="bg-emerald-500 h-full rounded-full" style={{ width: `${Math.min(100, Math.max(10, (150 - stats.latency) / 1.5))}%` }} />
              </div>
            </div>
            <p className="text-[8px] text-slate-500 font-semibold uppercase tracking-wider">+12ms vs. dün · WebSocket Bağlantısı</p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-1.5 text-slate-400">
            ...
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-300">Topluluk</h3>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl p-4 border border-white/[0.04] bg-white/[0.01] flex flex-col justify-between min-h-[90px]">
              <span className="text-[9px] font-black uppercase tracking-wider text-slate-500">Toplam Üye</span>
            ...
            <div className="rounded-xl p-4 border border-white/[0.04] bg-white/[0.01] flex flex-col justify-between min-h-[90px]">
              <span className="text-[9px] font-black uppercase tracking-wider text-slate-500">Gönderi - Yorum</span>
              ...
              <p className="text-[8px] text-slate-400">Son 24 saat içinde</p>
            </div>
            <div className="rounded-xl p-4 border border-white/[0.04] bg-white/[0.01] flex flex-col justify-between min-h-[90px]">
              <span className="text-[9px] font-black uppercase tracking-wider text-slate-500">Beğeni - Topluluk</span>
              ...
              <p className="text-[8px] text-slate-400">Etkileşim oranı %48</p>
            </div>
          </div>
        </div>

        {/* Right Column: Moderasyon Durumu */}
        <div className="space-y-3">
          <div className="flex items-center gap-1.5 text-slate-400">
            <Shield size={14} className="text-violet-400" />
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-300">Moderasyon Durumu</h3>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl p-4 border border-white/[0.04] bg-white/[0.01] flex flex-col justify-between min-h-[90px]">
              <span className="text-[9px] font-black uppercase tracking-wider text-slate-500">Bekleyen Rapor</span>
              <p className="text-2xl font-black text-white mt-1">{stats.openTickets}</p>
              <p className="text-[8px] text-rose-400 font-semibold">{stats.openTickets} yüksek öncelikli talep</p>
            </div>
            <div className="rounded-xl p-4 border border-white/[0.04] bg-white/[0.01] flex flex-col justify-between min-h-[90px]">
              <span className="text-[9px] font-black uppercase tracking-wider text-slate-500">Onay Bekleyen</span>
              <p className="text-2xl font-black text-white mt-1">0</p>
              <p className="text-[8px] text-slate-500">Tüm onaylar kapalı</p>
            </div>
            <div className="rounded-xl p-4 border border-white/[0.04] bg-white/[0.01] flex flex-col justify-between min-h-[90px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6">
          ...
          <div className="rounded-2xl p-5 border border-white/[0.06] bg-[#090912]/80 space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-1.5">
                ...
                <h4 className="text-xs font-black uppercase tracking-widest text-slate-300">Son 24 Saat Etkileşim</h4>
              </div>
          ...
          <div className="rounded-2xl p-5 border border-white/[0.06] bg-[#090912]/80 space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-white/5">
              <div className="flex items-center gap-1.5">
                ...
                <h4 className="text-xs font-black uppercase tracking-widest text-slate-300">Cohort Retention (Haftalık)</h4>
              </div>
              <span className="text-[10px] text-slate-500 font-bold">D7 Ortalama: %73</span>
            </div>
        ...
        {/* Right Column blocks */}
        <div className="space-y-6">
          {/* Raporlar Kuyruğu (Reports Queue) */}
          <div className="rounded-2xl p-5 border border-white/[0.06] bg-[#090912]/80 space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-white/5">
              <div className="flex items-center gap-1.5">
                <AlertTriangle size={14} className="text-rose-500" />
                <h4 className="text-xs font-black uppercase tracking-widest text-slate-300">Raporlar Kuyruğu</h4>
              </div>
              <span className="text-[10px] text-rose-500 font-bold">Tümü ({tickets.length})</span>
            </div>
            ...
            <div className="space-y-2.5 max-h-[290px] overflow-y-auto pr-1">
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider border select-none ${badgeColor}`}>
                      {ticket.status === 'open' ? 'Yüksek' : 'Orta'}
                    </span>
                  </div>
                )
              })}
              ...
              {tickets.length === 0 && (
                <div className="text-center py-10 text-xs text-slate-500 font-bold border border-dashed border-white/5 rounded-xl">
                  Aktif moderasyon raporu bulunmuyor.
                </div>
              )}
            </div>
          </div>

          {/* Canlı Aktivite (Live Activity & logs list) */}
          <div className="rounded-2xl p-5 border border-white/[0.06] bg-[#090912]/80 space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-white/5">
              <div className="flex items-center gap-1.5">
                <Activity size={14} className="text-emerald-500" />
                <h4 className="text-xs font-black uppercase tracking-widest text-slate-300">Canlı Aktivite</h4>
              </div>
              <span className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
            ...
            <div className="grid grid-cols-2 gap-3 items-center">
              ...
              <div className="p-3 rounded-xl border border-white/[0.03] bg-white/[0.01] flex items-center justify-between gap-2.5">
                <div className="min-w-0">
                  <span className="text-[8px] font-black uppercase tracking-wider text-slate-500">Çözüm Oranı</span>
              ...
              <div className="p-3 rounded-xl border border-white/[0.03] bg-white/[0.01] flex flex-col justify-between min-h-[56px]">
                <span className="text-[8px] font-black uppercase tracking-wider text-slate-500">Anlık Aktif</span>
                <div className="flex items-baseline gap-1 mt-1">
                  <span className="text-base font-black text-white">{stats.onlineUsers}</span>
                  <span className="text-[7px] font-bold text-slate-500 uppercase">Üye</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2.5">
              <div className="flex items-center justify-between text-[9px] text-slate-400 font-bold uppercase tracking-wider">
                <span>Son Moderasyon Aksiyonları</span>
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-[#090912]/80 p-5 space-y-4">
        <div className="pb-2.5 border-b border-white/5 flex items-center gap-1.5">
          <Shield size={14} className="text-violet-400" />
          <h4 className="text-xs font-black uppercase tracking-widest text-slate-300">Audit Log — Tüm Admin Aksiyonları</h4>
        </div>
          ...
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            >
              ...
              <div className="flex items-center justify-between pb-3 border-b border-white/5 flex-shrink-0">
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Kullanıcı Detayı</span>
                <button 
                  onClick={() => setSelectedUser(null)}
