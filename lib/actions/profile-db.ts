...
export async function saveProfileMetadata(userId: string, newMeta: ProfileUpdateMeta, newBioText?: string | null) {
  ...
  if (dbHasColumns || dbHasVerification) {
    ...
    // Keep accent_theme, last_session_id, is_setup_completed, show_xp, show_badges, and preferred_language in the bio suffix since there are no native columns for them
